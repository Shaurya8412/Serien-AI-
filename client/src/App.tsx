...
import { chatWithAI, analyzeImage, generateImage } from './services/geminiService';

function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}

type Tab = 'chat' | 'vision' | 'generate';
...
export default function App() {
  ...
  const handleGenerateSubmit = async () => {
    if (!genPrompt.trim() || loading) return;
    setLoading(true);
    setGeneratedImageUrl(null);

    try {
      const url = await generateImage(`A calming and therapeutic visualization of: ${genPrompt}`);
      setGeneratedImageUrl(url);
    } catch (error) {
      console.error(error);
      alert("I couldn't visualize that right now. Perhaps we can try a different description?");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className={cn("min-h-screen flex flex-col transition-colors duration-500", bgColor)}>
      ...
      <nav className="flex items-center justify-center gap-2 mb-8 p-1 bg-zinc-200/50 rounded-2xl self-center">
        {[
          ...
          { id: 'generate', icon: Palette, label: 'Mood Art' }
        ].map((tab) => (
          <button
            key={tab.id}
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'generate' && (
            <motion.div 
              ...
              className="flex-1 flex flex-col gap-6"
            >
              <div className="glass-panel rounded-3xl p-8 flex flex-col items-center gap-8">
                <div className="w-full max-w-lg space-y-4 text-center">
                  <h2 className="text-xl font-semibold text-zinc-900">Mood Visualization</h2>
                  <p className="text-sm text-zinc-500">Describe your current state of mind, and I'll create a calming visualization for you.</p>
                  
                  <div className="space-y-2 text-left">
                    <textarea 
                      value={genPrompt}
                      onChange={(e) => setGenPrompt(e.target.value)}
                      placeholder="e.g., A peaceful forest at dawn with soft sunlight filtering through the trees..."
                      className="w-full h-24 px-4 py-3 bg-zinc-100 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all resize-none"
                    />
                  </div>
                  <button 
                    onClick={handleGenerateSubmit}
                    disabled={!genPrompt.trim() || loading}
                    className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold text-lg hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center justify-center gap-3 shadow-lg shadow-indigo-200"
                  >
                    {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : <Palette className="w-6 h-6" />}
                    Visualize My Mood
                  </button>
                </div>

                <div className="w-full max-w-lg aspect-square rounded-3xl border-2 border-dashed border-zinc-200 bg-zinc-50 overflow-hidden relative group">
                  {generatedImageUrl ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                        <a 
                          ...
                          download="serenity-mood-art.png"
                      ...
                      {loading ? (
                        <div className="flex flex-col items-center gap-4">
                          <div className="relative">
                            ...
                            <Heart className="w-6 h-6 text-indigo-400 absolute -top-2 -right-2 animate-pulse" />
                          </div>
                          <p className="font-medium text-zinc-600">Crafting your visualization...</p>
                        </div>
                      ) : (
                        <>
                          <Palette className="w-16 h-16 mb-4 opacity-20" />
                          <p className="font-medium">Your mood art will appear here</p>
                          <p className="text-xs mt-2 italic">A visual representation of your inner peace</p>
                        </>
                      )}
                    </div>
                  )}
